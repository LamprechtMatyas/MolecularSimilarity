...
We use hashed ecfp fingerprints then we print the differences of representation of
...
def extract_neighbourhood_fragments(molecule: object, diameter: int, options: dict, nbits) -> list:
    ...
    info = {}
    size = diameter // 2
    AllChem.GetHashedMorganFingerprint(molecule, size, nBits=nbits, bitInfo=info)
    for element in info:
