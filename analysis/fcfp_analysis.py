...
We use fcfp fingerprints as bit vector then we print the differences of representation of
...
def extract_neighbourhood_fragments(molecule: object, diameter: int, options: dict, nbits) -> list:
    ...
    info = {}
    size = diameter // 2
    AllChem.GetMorganFingerprintAsBitVect(molecule, size, nBits=nbits, bitInfo=info, useFeatures=True)
    for element in info:
